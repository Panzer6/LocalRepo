...
class ATM(EasyFrame):
    ...
    def login(self):
        ...
        if self.account:
            ...
            self.messageBox(title = "WARNING", message = "(2) more attempts until police intervention.")
            self.error += 1
        elif self.error == 1:
            self.statusField.setText("Name and pin not found!")
            self.messageBox(title = "WARNING", message = "(1) more attempt until police intervention.")
            self.error += 1
        elif self.error == 2:
            self.statusField.setText("Name and pin not found!")
            self.messageBox(title = "STOP RIGHT THERE CRIMINAL SCUM", message = "Nobody breaks the law on my watch. Police are on the way.")
